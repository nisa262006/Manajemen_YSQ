...
test('Admin Berhasil Menambahkan dan Mengedit Jadwal', async ({ page }) => {
  ...
  await page.selectOption('#kelas-tingkatan', { label: 'Tahsin B - 0566' }); 
  await page.selectOption('#pengajar-tambah', { label: 'Ustadz Ahmad Fauzi' });
  ...
  // Mencari baris yang baru dibuat lalu klik ikon pensil
  const rowJadwal = page.locator('#jadwalBody tr').filter({ hasText: 'Tahsin B - 0566' });
