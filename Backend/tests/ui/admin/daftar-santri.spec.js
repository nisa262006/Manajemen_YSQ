...
test('Admin Berhasil Mencari dan Memfilter Daftar Santri', async ({ page }) => {
  ...
  // 7. Test Fitur Edit Santri
  console.log('Mencoba mengedit data santri...');
  const editBtn = firstRow.locator('.edit-btn');
  await editBtn.click();

  // Tunggu halaman detail santri
  await expect(page).toHaveURL(/.*detail-santri.*/);
  console.log('Berhasil masuk ke halaman detail santri.');
  ...
  const alamatBaru = 'Alamat Baru ' + Date.now();
  await page.fill('#alamat', alamatBaru);
  
  // Ubah status jika perlu
  await page.selectOption('#status-akun', 'aktif');

  console.log('Menyimpan perubahan santri...');
  
  // Handler untuk menutup alert secara otomatis (baik sukses maupun error)
  page.on('dialog', async dialog => {
    console.log(`Alert Muncul: [${dialog.type()}] ${dialog.message()}`);
    await dialog.accept();
  });

  await page.waitForTimeout(1000);
  await page.click('#btn-simpan-santri-footer');

  // Biasanya ada alert, kita tunggu redirect kembali ke daftar santri
  await expect(page).toHaveURL(/.*dashboard\/daftar-santri/, { timeout: 15000 });
  console.log('Alhamdulillah! Edit santri berhasil.');

  // 8. Test Fitur Ekspor Data
