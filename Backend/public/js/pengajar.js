...
async function loadPengajarData() {
  ...
  try {
    // 1️⃣ Ambil kelas pengajar
    const kelasRes = await fetchJSON(`${BASE_URL}/kelas/pengajar/me`, {
      headers: { Authorization: `Bearer ${token}` }
    });

    const kelasList = Array.isArray(kelasRes?.data)
      ? kelasRes.data
    ...
    if (!kelasList.length) {
      console.warn("Pengajar belum punya kelas");
      return;
    }

    const kelas = kelasList[0];
    document.querySelector(".nama_kelas").textContent =
      kelas.nama_kelas ?? "-";

    // 2️⃣ Ambil jadwal pengajar
    const jadwalRes = await fetchJSON(`${BASE_URL}/jadwal/pengajar/me`, {
      headers: { Authorization: `Bearer ${token}` }
    });

    const jadwalList = Array.isArray(jadwalRes?.data)
      ? jadwalRes.data
      : Array.isArray(jadwalRes)
        ? jadwalRes
        : [];

    console.log("ALL JADWAL:", jadwalList);

    // ================= FILTER HARI INI =================
    const hariIni = new Date().toLocaleDateString("id-ID", { weekday: "long" });
    const hariNormalized =
      hariIni.charAt(0).toUpperCase() + hariIni.slice(1);

    const jadwalHariIni = jadwalList.filter(j =>
      j.hari?.toLowerCase() === hariNormalized.toLowerCase()
    );

    console.log("JADWAL HARI INI:", jadwalHariIni);

    // ================= RENDER DASHBOARD =================
    renderDashboardJadwal(jadwalHariIni);

    document.querySelector(".jadwal_kelas").textContent =
      jadwalHariIni.length
        ? jadwalHariIni[0].hari
        : "Tidak ada jadwal hari ini";

    // 3️⃣ Ambil detail kelas (jumlah santri)
    const detail = await fetchJSON(
      `${BASE_URL}/kelas/pengajar/detail/${kelas.id_kelas}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );

    const santri = Array.isArray(detail?.santri)
      ? detail.santri
      : [];

    document.querySelector(".jumlah_santri").textContent = santri.length;
